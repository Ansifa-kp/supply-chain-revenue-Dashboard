...
DB_CONFIG = {
    "host": "localhost",
    "user": "root",           # <--- Use your user name of sql
    "password": "MySQL@123",  # <--- Update this
    "database": "sakila"
}

# --- 2. The "Clean Root" SQL Query ---
# This structure is the ONLY way to prevent the 11,22 math
# duplication issue
sql_query = """
SELECT
    f.film_id AS ID,
...
def run_analytics_system():
    try:
        ...
        # FIX: Was split across two lines — joined onto one line
        ...
        # FIX: These lines were inconsistently indented — all now inside try block
        ...
        # FIX: writer.close() and everything below was outside try — now correctly inside
        writer.close()
